...
function parseArgs(argv) {
  const args = { password: null, db: null };
...
function main() {
  ...
  if (args.help) {
    console.log(
      "Usage: node tools/reset-all-users-passwords.js [--password <newPassword>] [--db <path>]",
    );
    console.log("Tip: pipe the password via stdin to avoid it showing in history.");
    process.exit(0);
  }
  ...
  const run = async () => {
    ...
    const users = db.exec("SELECT id, username FROM users ORDER BY id ASC");
    if (!users || users.length === 0 || users[0].values.length === 0) {
      console.error("ERROR: No users found to reset.");
      process.exit(1);
    }
    ...
    console.log("Done: reset password for all users.");
  };
